...
void re_match_here(char *pat, char *text, SPAN *span)
{
    ...
    if (pat[0] == '[')
    {
        ...
        {
            if (pat[close + 2] == '?')
                return re_match_star(&bucket, pat + close + 3, text, span, 0);
            else
                return re_match_star(&bucket, pat + close + 2, text, span, 1);
            return;
        }
        else if (pat[close + 1] == '+')
        {
            if (pat[close + 2] == '?')
                return re_match_plus(&bucket, pat + close + 3, text, span, 0);
            else
                return re_match_plus(&bucket, pat + close + 2, text, span, 1);
            return;
        }
        else if (pat[close + 1] == '?')
        {
            if (pat[close + 2] == '?')
                return re_match_question(&bucket, pat + close + 3, text, span, 0);
            return re_match_question(&bucket, pat + close + 2, text, span, 1);
        }
    }
    span->valid = 0;
    return;
